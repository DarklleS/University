...
void MainWindow::intError(int idCol, int size) // Функция демонстрации ошибки при неверно введеном значении в ячейку таблицы
{
    ...
    ui->label_messege->setText("Введите верные данные в ячейки!");
    ui->label_messege->setStyleSheet("color: white; background-color: red; font-size: 32px");
    showMessege();
...
int MainWindow::mainArray(int arr[], int size) // Функция главного массива берущего значения из ячеек таблицы
{
    ...
    int numCol = size*2; // Переменная кол-ва столбцов таблицы
...
void MainWindow::on_pushButton_enter_clicked() // Функция ввода кол-ва столбцов
{
    ...
    int numCol = ui->tableWidget->columnCount();

    for (int j = 0; j < numCol; j++)
    {
        if (ui->tableWidget->item(0, j) != NULL)
            ui->tableWidget->item(0, j)->setText(NULL);
    }

    ui->tableWidget->setColumnCount(NULL);

    int col = ui->lineEdit_col->text().toInt(); // Переменная считывания значения кол-ва столбцов

    if (col<=100000 && col >=1) // Проверка на диапозон введенных значений
    {
        ui->tableWidget->setColumnCount(col);
        showMessege();
        ui->label_messege->setText("Добавление " + QString::number(col) + " столбцов прошло успешно!");
        ui->label_messege->setStyleSheet("background-color: green; font-size: 24px; color: white");
    }
    else
    {
        showMessege();
        ui->label_messege->setText("Введите кол-во от 1 до 100000 столбцов!");
    }
    numCol = NULL;
    col = NULL;
}
...
void MainWindow::on_pushButton_bubbleSort_clicked() // Функция сортировки эллементов таблицы алгоритмом - сортировка пузярьком
{
    ...
    if (numCol==0) // Проверка на пустоту таблицы
    {
        showMessege();
        ui->label_messege->setText("Введите кол-во столбцов!");
    }
    ...
    if (numCol >= 1000)
    {
        reply = QMessageBox::question(this, "Предупреждение", "Данная сортировка столь большой таблицы может занять много времени. Вы согласны?", QMessageBox::Yes|QMessageBox::No);
        ...
        if (reply == QMessageBox::Yes)
        {
            if (flag) // Проверка на верность введенных данных в массиве
            {
                ...
                else
                {
                    ...
                    ui->label_messege->setStyleSheet("background-color: gold; font-size: 22px; color: white");
                }
            }
        }
    }
    ...
    if (flag) // Проверка на верность введенных данных в массиве
    {
        ...
        if (correct == 1)
        {
            if (flagNum)
            {
                if (count != 1)
                {
                    ...
                    if (bin != -1)
                    {
                        ...
                        ui->label_messege->setText("Бинарный поиск прошел успешно!");
                        ui->label_messege->setStyleSheet("background-color: green; font-size: 24px; color: white");
                        ...
                        ui->label_searchIndex->setStyleSheet("background-color: #333; color: white");
                        ui->label_searchDup->setStyleSheet("background-color: #333; border-radius: 10px; color: white");
                    }
                    else
                    {
                        showMessege();
                        ...
                        ui->label_searchDup->setVisible(false);
                        ui->label_searchIndex->setVisible(false);
                        ui->label_messege->setText("Число " + QString::number(k) + " было нe найдено!");
                ...
                else if (count == 1)
                {
                    ...
                    if (n > 0)
                    {
                        ...
                        ui->label_searchDup->setVisible(true);
                        ui->label_searchIndex->setVisible(true);
                        ui->label_messege->setText("Линейный поиск прошел успешно!");
                        ui->label_messege->setStyleSheet("background-color: green; font-size: 24px; color: white");
            ...
            else
            {
                ...
                ui->label_searchDup->setVisible(false);
                ui->label_searchIndex->setVisible(false);
                ui->label_messege->setText("Введите верное число в поле поиска!");
                ui->label_messege->setStyleSheet("background-color: red; font-size: 24px; color: white");
            }
        }
        else
